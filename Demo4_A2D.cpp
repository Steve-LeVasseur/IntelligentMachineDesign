...
volatile uint8_t FirstLineStr[21] =  "S1:                 ";
volatile uint8_t SecondLineStr[21] = "S2:                 ";
volatile uint8_t ThirdLineStr[21] =  "Button: O           ";
...
//Select Analog Port 0 and Internal Reference Voltage;
	...
	OCR1A = 19999; //OCR1A = 16Mhz/N*Deltat-1 = 16Mhz/64*.1-1
...
{
	ADCSRA |= (1<<ADSC); //Start Conversion
	PORTB ^= 0x01; //Toggle Pin PB0 (breadcrumb)
	display_counter++;
	button_temp = !((1<<PIND5)&PIND);
	if (display_counter>49){
	
	display_counter = 0;
	combined = high_out1 * 256;
	combined = combined + low_out1;
	...
	combined = high_out2 * 256;
	combined = combined + low_out2;
...
ISR (ADC_vect)
{
	...
	//Do something with A/D result (How should we store the data?)
	
	// need something that checks which sensor we're reading to save to different variables
	if (which_ad == 1)
	{
		low_out1 = ADCL;
		high_out1 = ADCH;
	...
	// here we would switch to second sensor and activate another conversion
